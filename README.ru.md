...
- **Интерактивное всплывающее окно (Flyout):**
  - Отображение текущего процента заряда, статуса зарядки (плашка `⚡ Charging`), прогресс-бара и модели гарнитуры.
...
- **Индикация зарядки:** Отображение бейджа зарядки на иконке панели задач, зеленый прогресс-бар и детальный статус зарядки.
- **Настраиваемость и Fluent-темы:** Скрытие индикатора при отключении гарнитуры, выбор стиля отображения (проценты или иконка), полноценная поддержка светлой и тёмной темы Windows 11 для оверлея и контекстного меню.
- **Автозагрузка:** Автоматический запуск вместе с Windows.
- **Уведомления:** Всплывающие предупреждения при низком уровне заряда (<= 20%).
- **Контроль логирования и ротация:** Запись в `debug.log` отключена по умолчанию в релизной сборке (включается через `--debug` или отладочную версию `HeadsetControlTaskbarBatteryIndicatorDebug.exe`) с автоматической ротацией файлов до 1 МБ.
